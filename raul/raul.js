...
function setup() {
  ...
  path = font.getPath("abcdefghijklmn", 0, 0, 100);
}
...
function draw() {
  ...
  background(148, 157, 142);
  noStroke();
  fill(mouseX/2,mouseX,mouseY/3,100);
  push();
  translate(25, 150);
  drawContours(
    groupByContour(
      commandTransform(path.commands, function(x, y) {
        let newX = x + sin((x*0.25) + (mouseX*0.11));
        let newY = y + cos((y*0.15) + (mouseY*0.17));
        return [newX, newY];
      })
    )
  );
